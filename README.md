...
| v2board | Supported | Implemented through the v2board-family adapter. Legacy schema differences are handled for plans, Trojan, and Hysteria nodes. |
...
For xiaov2board and v2board, the migrator currently covers:
...
- Nodes: `servers` and `nodes` records are generated from v2board-family protocol tables.
...
- Supported v2board node tables: `v2_server_vmess`, `v2_server_trojan`, `v2_server_shadowsocks`, and `v2_server_hysteria`.
...
- xiaov2board and v2board are currently implemented through the v2board-family adapter path.
- Xboard, PPanel, and SSPanel-UIM are part of the design roadmap but should not be treated as supported until their adapters are implemented and tested.
...
| v2board | 已支持 | 通过 v2board-family adapter 实现；已兼容普通 v2board 的套餐、Trojan、Hysteria 字段差异。 |
| Xboard | 规划中 | 暂未实现。 |
| PPanel | 规划中 | 暂未实现。 |
...
### xiaov2board / v2board 已支持功能

当前已经支持 xiaov2board 和 v2board 迁移到 NPanel，覆盖以下数据：
...
- 节点：从 v2board-family 各协议节点表生成 NPanel `servers` 和 `nodes`。
...
- 普通 v2board 节点表范围：`v2_server_vmess`、`v2_server_trojan`、`v2_server_shadowsocks`、`v2_server_hysteria`。
...
- 当前完整支持的是 xiaov2board / v2board 到 NPanel。
- Xboard、PPanel、SSPanel-UIM 等面板属于后续规划，未实现完整 adapter 前不要按已支持能力使用。
